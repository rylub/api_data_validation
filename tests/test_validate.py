...
def test_validate_crypto_data_valid():
    ...
    data = {'bitcoin': {'usd': 20000}, 'ethereum': {'usd': 1500}}
    coins_list = ['bitcoin', 'ethereum']
    report = validate_crypto_data(data, coins_list)
...
def test_validate_crypto_data_invalid():
    ...
    data = {'bitcoin': {'usd': -1000}}  # Negative value is invalid
    coins_list = ['bitcoin']
    report = validate_crypto_data(data, coins_list)
...
def test_validate_crypto_data_missing():
    ...
    report = validate_crypto_data(None, ['bitcoin'])
    assert report['status'] == 'FAIL'
    assert 'No data received from API.' in report['details']
